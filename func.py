...
def start(update, context):
    ...
    if TG_ID ==  2071126215:
                ...
                context.bot.send_message(chat_id=user_id, text='Admin panel',    reply_markup=InlineKeyboardMarkup([knop, Kmo,Km]))
...
def next_func(update, context):
    ...
    stage_ = stage_[0][0]
    lang_ = lang_[0][0]
    if message.lower() == 'davom etish>>>' and stage_ == 2 or message.lower() == 'далее>>>' and stage_ == 2 or stage_== 50 or  message == dct[lang_][0] and stage_ == 2:
    ...
    if x!=1 and stage_==3:
        if message== 'davom etish>>>' or message== 'далее>>>':
             ...
             sayt = [InlineKeyboardButton(text='📨Telegram', url='https://t.me/mazzami_sizlarga_mazza'),
                     InlineKeyboardButton(text='📸Instagram', url='https://instagram.com/mazzami_sizlarga_mazzami?utm_medium=copy_link'),
                     InlineKeyboardButton(text='📺YouTube',
                                          url='https://youtube.com/channel/UClwA3tMaxWmTfDjcniyVh6Q'),
                     ]
             syt = [InlineKeyboardButton(text=' 🌐Website', url='http://mazzami-sizlarga.uz/')]
             context.bot.send_message(chat_id=user_id, text=dct[lang_][10],reply_markup=InlineKeyboardMarkup([sayt,syt]))
             context.bot.send_message(chat_id=user_id, text=dct[lang_][2])
        cur.execute(stagee.format('{}', user_id).format(3))
        connect.commit()
